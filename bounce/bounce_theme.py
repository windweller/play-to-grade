"""
Bounce but with invariances, including assets
"""
...
class Theme(object):
    ...
    def __init__(self):
        ...
        hardcourt_addr = {
            'background': os.path.join(curr_dir, 'assets/background.png'),
            'paddle': os.path.join(curr_dir, './assets/paddle.png'),
            'wall': os.path.join(curr_dir, './assets/wall.png'),
            'ball': os.path.join(curr_dir, './assets/ball.png'),
        }
        retro_addr = {
            'background': os.path.join(curr_dir, './assets/retro_background.png'),
            'paddle': os.path.join(curr_dir, './assets/retro_paddle.png'),
            'wall': os.path.join(curr_dir, './assets/retro_wall.png'),
            'ball': os.path.join(curr_dir, './assets/retro_ball.png')
        }
...
class Background(object):
    ...
    def draw(self, screen):
        ...
        # HARDCOURT = 'hardcourt'
        # RETRO = 'retro'
        screen.blit(eval('self.' + self.curr_theme + '_sprite'), (0, 0))

    def load_assets(self):
        img_path = theme_control.get_img_path(HARDCOURT, 'background')
        img = pygame.image.load(img_path)
        img = pygame.transform.scale(img, (screen_width, screen_height))
        self.hardcourt_sprite = img.convert()

        img_path = theme_control.get_img_path(RETRO, 'background')
        img = pygame.image.load(img_path)
        img = pygame.transform.scale(img, (screen_width, screen_height))
        self.retro_sprite = img.convert()
...
class Paddle(object):
    ...
    def load_assets(self):
        self.hardcourt_sprite = self.create_sprite(HARDCOURT)
        self.retro_sprite = self.create_sprite(RETRO)

        new_sprite = self.get_current_sprite()
        self.canvas.all_sprites.add(new_sprite)

    def create_sprite(self, theme):
        img_path = theme_control.get_img_path(theme, 'paddle')
        img = pygame.image.load(img_path)
        ...
        sprite.rect.x, sprite.rect.y = self.get_initial_x_y()

        return sprite

    def get_current_sprite(self):
        return eval('self.' + self.curr_theme + '_sprite')

    def set_theme(self, theme_str):
        # this does not affect the current ball
        if theme_str != self.curr_theme:
            old_sprite = self.get_current_sprite()
            self.canvas.all_sprites.remove(old_sprite)
            self.curr_theme = theme_str
            new_sprite = self.get_current_sprite()
            new_sprite.rect = old_sprite.rect
            self.canvas.all_sprites.add(new_sprite)

            self.obj = new_sprite.rect
    ...
    def setup_paddle(self):
        ...
        # return paddle
        return self.get_current_sprite().rect
...
class Ball(object):
    ...
    def create_sprite(self, theme):
        if theme == HARDCOURT:
            ...
            scale_size = (27, 27)
        img_path = theme_control.get_img_path(theme, 'ball')
        img = pygame.image.load(img_path)
        ...
        self.init_x, self.init_y = self.get_initial_x_y()
        sprite.rect.x, sprite.rect.y = self.init_x, self.init_y

        return sprite

    def get_initial_x_y(self):
        # location is fixed, but velocity isn't
        ball_x = self.canvas.get_canvas_width() / 2 - BALL_RADIUS
        ball_y = self.canvas.get_canvas_height() / 2 - BALL_RADIUS
...
class ScoreBoard(object):
    ...
    def draw(self):
        ...
        x_pos = (screen_width - 110) // 2

        self.canvas.create_white_text(x_pos, 50, text, "comicsansms", 30, self.key)

    def update(self):
        text = "Score " + str(self.own) + " : " + str(self.opponent)
        self.canvas.set_white_text(self.key, text)
...
class WallImage(object):
    ...
    def create_sprite(self, theme):
        img_path = theme_control.get_img_path(theme, 'wall')
        img = pygame.image.load(img_path)
        img = pygame.transform.scale(img, (screen_width, screen_height))
...
class Bounce(object):
    ...
    def __init__(self, program):
        ...
        self.bg = Background()
        self.wall_image = WallImage(canvas)
    ...
    def run(self, debug=False):
        ...
        while not self.score_board.game_over():
            ...
            self.bg.draw(self.canvas.screen)
            self.canvas.update_only_objects()
            self.canvas.flip()
            time.sleep(1 / 50)
...
if __name__ == '__main__':
    program = Program()
    program.load('programs/set_themes.json')
